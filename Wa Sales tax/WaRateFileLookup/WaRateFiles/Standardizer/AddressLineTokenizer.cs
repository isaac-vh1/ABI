...
namespace WaRateFiles.Standardizer
{
	...
	{
		private static StringTable m_strtab = new StringTable();
		private static ObjectPool<Lex> m_lexPool = new ObjectPool<Lex>();
		private static ObjectPool<List<AddressToken>> m_listPool = new ObjectPool<List<AddressToken>>();
		...
		public void Init(string street)
		{
			...
			LexTokenType token;
			Lex lexer = m_lexPool.Get();
			lexer.Init(street);

			List<AddressToken> tokens = m_listPool.Get();
			tokens.Clear();
